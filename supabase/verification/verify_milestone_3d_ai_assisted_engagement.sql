WITH substantive_results AS (
  ...
  SELECT 'FUNCTION', 'public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)', 'human_approval_required', 'EXISTS',
         CASE WHEN to_regprocedure('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)') IS NOT NULL THEN 'EXISTS' ELSE 'MISSING' END,
         CASE WHEN to_regprocedure('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)') IS NOT NULL
                 AND lower(pg_get_functiondef('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)'::regprocedure)) LIKE '%p_requires_human_approval and p_human_approval_granted = false%'
                 AND lower(pg_get_functiondef('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)'::regprocedure)) LIKE '%human approval is required before this outreach can be sent%' THEN 'PASS' ELSE 'FAIL' END,
         CASE WHEN to_regprocedure('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)') IS NOT NULL
                 AND lower(pg_get_functiondef('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)'::regprocedure)) LIKE '%p_requires_human_approval and p_human_approval_granted = false%'
                 AND lower(pg_get_functiondef('public.evaluate_engagement_send_eligibility(uuid,text,text,uuid,uuid,text,boolean,boolean,boolean,boolean,boolean,boolean,text,boolean,boolean,boolean,boolean,boolean)'::regprocedure)) LIKE '%human approval is required before this outreach can be sent%' THEN 'Human approval is required in the eligibility gate and autonomous delivery is blocked.' ELSE 'Human approval gate is missing or autonomous delivery is allowed.' END
), all_results AS (
