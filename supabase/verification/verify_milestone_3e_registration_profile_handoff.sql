...
WITH substantive_results AS (
  ...
  UNION ALL
  SELECT 'FUNCTION', 'public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)', 'function_exists', 'EXISTS',
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL THEN 'EXISTS' ELSE 'MISSING' END,
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL THEN 'PASS' ELSE 'FAIL' END,
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL THEN 'Function exists in the migration signature set.' ELSE 'Function is missing from the migration signature set.' END
  UNION ALL
  ...
  UNION ALL
  SELECT 'FUNCTION', 'public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)', 'function_exists', 'EXISTS',
         CASE WHEN to_regprocedure('public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)') IS NOT NULL THEN 'EXISTS' ELSE 'MISSING' END,
         CASE WHEN to_regprocedure('public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)') IS NOT NULL THEN 'PASS' ELSE 'FAIL' END,
         CASE WHEN to_regprocedure('public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)') IS NOT NULL THEN 'Function exists in the migration signature set.' ELSE 'Function is missing from the migration signature set.' END
  UNION ALL
  ...
  UNION ALL
  SELECT 'FUNCTION', 'public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)', 'release_3c_engagement_thread_message_linkage', 'thread_message_linkage',
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL THEN 'PRESENT' ELSE 'MISSING' END,
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL AND lower(pg_get_functiondef('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)'::regprocedure)) LIKE '%related_thread_id%' AND lower(pg_get_functiondef('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)'::regprocedure)) LIKE '%related_message_id%' THEN 'PASS' ELSE 'FAIL' END,
         CASE WHEN to_regprocedure('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)') IS NOT NULL AND lower(pg_get_functiondef('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)'::regprocedure)) LIKE '%related_thread_id%' AND lower(pg_get_functiondef('public.record_registration_journey_event(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,text,text,jsonb)'::regprocedure)) LIKE '%related_message_id%' THEN 'Journey events preserve engagement thread and message linkage.' ELSE 'Journey event thread/message linkage is incomplete.' END
  UNION ALL
  ...
  SELECT 'INDEX', 'public.brilliant_directories_sync_events', 'provider_sync_event_idempotency', 'idempotent_sync',
         CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'brilliant_directories_sync_events' AND indexname = 'ux_brilliant_directories_sync_events_idempotency') THEN 'PRESENT' ELSE 'MISSING' END,
         CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'brilliant_directories_sync_events' AND indexname = 'ux_brilliant_directories_sync_events_idempotency') AND lower(pg_get_functiondef('public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)'::regprocedure)) LIKE '%on conflict%' THEN 'PASS' ELSE 'FAIL' END,
         CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'brilliant_directories_sync_events' AND indexname = 'ux_brilliant_directories_sync_events_idempotency') AND lower(pg_get_functiondef('public.ingest_brilliant_directories_sync_event(uuid,uuid,uuid,uuid,text,text,text,jsonb,text)'::regprocedure)) LIKE '%on conflict%' THEN 'Provider sync events are idempotent and deduplicated by payload hash.' ELSE 'Provider sync event idempotency is missing.' END
  UNION ALL
